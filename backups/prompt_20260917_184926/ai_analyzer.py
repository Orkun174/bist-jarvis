...
from core.ml_contract import TARGET_DEFINITION, LEGACY_TARGET_DEFINITION, PREVIOUS_TARGET_DEFINITION
SUPPORTED_TARGETS = {TARGET_DEFINITION, LEGACY_TARGET_DEFINITION, PREVIOUS_TARGET_DEFINITION}
...
The current training target is Close[t+5] > Close[t]: ANY strictly positive
terminal return over five trading sessions. Flat returns are class zero.
An older model may target >=3% over five sessions or >=2% over three; use its saved
target exactly. Do not relabel an old score as the new event.
For the current directional target this is the score for any positive terminal
return; it is not an intrahorizon price touch. For older threshold targets,
class zero also includes small gains. Flat returns prevent interpreting class
zero as strictly a decline in either case. Inspect the calibration field: identity means uncalibrated; sigmoid means a
separate chronological calibration segment was used. Neither guarantees accuracy.
The acceptance rule may be validation_auc >0.505; passing this historical
threshold is not proof of a tradable or causal edge. Inspect the actual report.
