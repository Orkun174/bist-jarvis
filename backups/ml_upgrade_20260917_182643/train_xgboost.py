...
CONFIGS = [
    {'name':'shallow', 'max_depth':1, 'n_estimators':140, 'min_child_weight':80, 'window':None},
    {'name':'regularized', 'max_depth':2, 'n_estimators':220, 'min_child_weight':60, 'window':None},
    {'name':'recent', 'max_depth':2, 'n_estimators':220, 'min_child_weight':60, 'window':504},
]
...
def classifier(config):
    return XGBClassifier(objective='binary:logistic', eval_metric='logloss',
        ...
        min_child_weight=config['min_child_weight'], learning_rate=.025,
        reg_lambda=20., reg_alpha=.5, subsample=.85, colsample_bytree=.9,
...
def evaluate(rows):
    ...
    report={**candidate,'baseline_log_loss':baseline_metrics['frequency']['log_loss'],
