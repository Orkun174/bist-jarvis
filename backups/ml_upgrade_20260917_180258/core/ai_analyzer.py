...
Close[t+3] / Close[t] - 1 >= 2%, where t+3 means three trading sessions.
It is not the probability of any positive return or an intrahorizon 2% touch.
The negative class includes gains smaller than 2%; a low score alone does not
predict a decline. The score is uncalibrated and is not a guaranteed success rate.
...
def _load_model_cached(path: str, modified_ns: int, size: int):
    ...
    if (
        ...
        or len(names) != 24
        or len(set(names)) != 24
        or set(names) != set(MODEL_FEATURE_COLUMNS)
    ...
    if target is not None and target != TARGET_DEFINITION:
...
def predict_uptrend(ticker: str, snapshot: dict) -> dict:
    ...
    booster = model.get_booster()
    matrix, provenance = build_inference_matrix(snapshot, booster.feature_names)
