...
CONFIGS = [
    {'name':'directional', 'max_depth':6, 'n_estimators':400, 'min_child_weight':60, 'window':None},
    {'name':'directional_recent', 'max_depth':6, 'n_estimators':400, 'min_child_weight':60, 'window':504},
]
...
def classifier(config):
    # AUC is the monitoring/selection metric; the differentiable training objective remains logistic.
    return XGBClassifier(objective='binary:logistic', eval_metric='auc',
        ...
        min_child_weight=config['min_child_weight'], learning_rate=.02,
...
def fit_candidate(rows, config, scope):
    ...
    # Train all 400 rounds; validation_0-auc is monitored, with no early stopping.
    # Identity calibration preserves the actual tree ranking and prevents a
    # zero-slope sigmoid from converting all outputs to the global prior.
    model.fit(matrix(fit,scope), fit.Target,
              eval_set=[(matrix(cal,scope), cal.Target)], verbose=False)
    ...
    return model,spec,{'fit_end':str(fit.Target_Date.max().date()),'calibration_start':str(cut.date()),
        ...
        'class_weighting':'disabled','fit_positive_count':int((fit.Target==1).sum()),
        ...
        for config in CONFIGS:
            for scope in ('full',):
                ...
                for method in ('identity',):
                    ...
                    auc=metrics(valid.Target,p)['roc_auc']
                    if auc is None or not np.isfinite(auc):
                        raise ValueError('Every validation fold requires both classes')
                    scores.setdefault(key,[]).append(auc)
        print('Completed development fold',fold,flush=True)
    ranked=sorted(scores,key=lambda k:np.mean(scores[k]),reverse=True)
    selected=ranked[0]
    name,scope,method=selected.split(':')
    return next(c for c in CONFIGS if c['name']==name),scope,method,{
        'selected':selected,'selection_metric':'roc_auc','maximize':True,'fold_aucs':scores,'ranking':ranked}
...
def accept_validation(auc):
    return bool(auc is not None and np.isfinite(auc) and auc > 0.53)
...
def evaluate(rows):
    ...
    # Requested acceptance policy: AUC > 0.53. Probability-quality metrics
    # remain visible diagnostics and do not override this explicit criterion.
    discrimination_ok=accept_validation(candidate['roc_auc'])
    passes=discrimination_ok
    report={**candidate,'target_definition':data.TARGET_DEFINITION,
        ...
        'target_comparison':'>','validation_auc':candidate['roc_auc'],
        'raw_validation_auc':metrics(test.Target,raw)['roc_auc'],
        'acceptance_rule':'validation_auc > 0.53',
...
def save_candidate(rows,report,quality,output):
    ...
    model.get_booster().set_attr(feature_columns=json.dumps(cols),
        ...
        target_comparison='>',class_weighting='disabled',acceptance_rule='validation_auc > 0.53',
...
def main():
    ...
    print(json.dumps({k:report[k] for k in ('validation_auc','raw_validation_auc','prediction_std','log_loss','baseline_log_loss','promotion_passed')},indent=2))
