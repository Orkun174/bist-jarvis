...
SYSTEM_PROMPT = (
    "You are a quant synthesizer. Evaluate the XGBoost probability against "
    "the dynamic threshold context. Formulate a final Conviction Score "
    "(High/Medium/Low) based strictly on the confluence of the top 3 SHAP "
    "technical drivers and the VIX macro regime. Output ONLY a valid JSON "
    "with 'conviction' and 'rationale' keys."
)
...
def prepare_latest_inference_bar(
    ...
    """Repair only the latest inference bar, without mutating cached history.
    ...
    estimated = not math.isfinite(volume) or volume <= 0
    source_date = None
    if estimated:
        # Use only earlier genuine source observations; no backfill or future data.
        previous = pd.to_numeric(
            equity.loc[equity.index < date, "Volume"], errors="coerce"
        ).sort_index()
        previous = previous.loc[np.isfinite(previous) & previous.gt(0)]
        if previous.empty:
            raise ValueError("No historical positive volume is available to estimate the latest bar.")
        volume = float(previous.iloc[-1])
        source_date = previous.index[-1].date().isoformat()
        repaired.at[date, "Volume"] = volume
    ...
    return repaired, {
        ...
        "volume_fallback_method": "previous_positive_observation" if estimated else None,
    }
...
def query_ollama(context: Mapping[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(parsed, dict) or set(parsed) != {
        ...
        raise ValueError("Invalid synthesizer JSON contract.")
    if parsed["conviction"] not in {"High", "Medium", "Low"}:
        raise ValueError("Invalid conviction label.")
    if (
        not isinstance(parsed["rationale"], str)
        or not parsed["rationale"].strip()
        or len(parsed["rationale"]) > 4000
    ):
        raise ValueError("Invalid synthesis rationale.")
...
def analyze(
    ...
    try:
        ...
        result["Analysis"] = {
            "Sentiment": "Neutral",
            "Technical_Context": f"Statistical inference unavailable: {exc}",
            "News_Impact": "The quantitative synthesis does not evaluate news.",
            "Conviction_Score": 5,
        }
        return result
    ...
    try:
        ...
        synthesis = {
            "conviction": "Low",
            "rationale": "Qwen synthesis unavailable; statistical evidence remains.",
        }

    signal = int(evidence["signal"])
    if signal == 0:
        sentiment = "Neutral"
        score = 5
        rationale = (
            f"{INSUFFICIENT_SIGNAL}. Probability lies inside the "
            "VIX-adjusted ambiguity zone. " + synthesis["rationale"]
        )
    else:
        sentiment = "Bullish" if signal > 0 else "Bearish"
        # Legacy UI numeric scores are directional adapters, not calibrated
        # confidence probabilities. Quant_Synthesis retains the exact new schema.
        scores = {
            1: {"Low": 6, "Medium": 7, "High": 9},
            -1: {"Low": 4, "Medium": 3, "High": 2},
        }
        score = scores[signal][synthesis["conviction"]]
        rationale = synthesis["rationale"]

    result["Analysis"] = {
        "Sentiment": sentiment,
        "Technical_Context": rationale,
        "News_Impact": "The quantitative synthesis does not evaluate news.",
