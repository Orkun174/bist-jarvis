...
try:
    ...
    raise ImportError(
        "XGBoostPruningCallback requires the optuna-integration xgboost extra."
    ) from exc
...
class FirstFoldPruningCallback(XGBoostPruningCallback):
    """Use Optuna's XGBoost integration for the first-fold utility monitor."""

    def __init__(self, trial: optuna.Trial) -> None:
        super().__init__(trial, "validation_0-selective_cost")
    ...
    def after_iteration(
        ...
        # All trials get a warm-up before the integration can prune them.
        if epoch < 49:
            return False
        return bool(super().after_iteration(model, epoch, evals_log))
...
def sample_parameters(trial: optuna.Trial) -> dict[str, Any]:
    return {
        ...
        "learning_rate": trial.suggest_float(
            "learning_rate", 0.008, 0.08, log=True
        ),
...
def evaluate_cpcv(
    ...
    for fold_number, split in enumerate(cv.split(rows)):
        ...
        def selective_cost(
            ...
            del actual
            returns, coverage, _ = portfolio_returns(validation, predicted)
            return coverage_adjusted_sortino(
                risk_ratios(returns)["sortino"], coverage
            )

        # Despite its identifier, selective_cost is a utility: the Optuna study
        # maximizes it. No XGBoost early-stopping metric direction is involved.
        callbacks = (
            [FirstFoldPruningCallback(trial)]
            if trial is not None and fold_number == 0 else None
        )
        model = make_model(
            parameters,
            callbacks=callbacks,
            evaluation_metric=(
                selective_cost if callbacks is not None else "logloss"
            ),
        )
        fit_options: dict[str, Any] = {}
        if callbacks is not None:
            fit_options["eval_set"] = [
                (matrix.iloc[split.test], validation["Target"])
            ]
            fit_options["verbose"] = False

        model.fit(
            matrix.iloc[split.train],
            training["Target"],
            **fit_options,
        )
        ...
        utility = coverage_adjusted_sortino(ratios["sortino"], coverage)

        # Explicit first-fold pruning covers zero-signal/negative-expectancy
        # trials even when the median pruning callback has too few peer trials.
        if trial is not None and fold_number == 0:
            if coverage == 0.0 or expectancy <= 0.0:
                raise optuna.TrialPruned(
                    "First fold has zero signals or nonpositive expectancy."
                )

        objectives.append(utility)
        records.append(
            {
    ...
    # Every fold participates equally; low-coverage folds cannot disappear
    # through a trade-count-weighted average.
...
def main() -> None:
    ...
    # Retain the established selective policy and require the newly requested
    # coverage and net-expectancy objectives on the untouched audit as well.
    ...
    if arguments.promote and promotion_passed:
        atomic_promote(candidate, experiment)
    elif arguments.promote:
        LOGGER.warning("Audit gate failed; active weights remain unchanged.")
