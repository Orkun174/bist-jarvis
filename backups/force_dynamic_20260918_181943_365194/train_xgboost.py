...
def evaluate_cpcv(
    ...
    # Complete inactive trials with a finite score so TPE learns to avoid them.
    # Active trials retain the original coverage-penalized Sortino objective.
    if all(record["coverage"] == 0.0 for record in records):
        return -100.0, records
