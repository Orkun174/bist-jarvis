...
METHODOLOGY = {
    ...
    "XGBoost · five-session training target": (
        "New training scores Close[t+5] > Close[t], using local "
        "technical indicators and macro features.",
        "XGB_Prob is a model percentage score; its calibration status is shown. A low score does not "
        "necessarily predict a decline: unchanged prices are also class 0. Older models retain their saved targets.",
...
def render_hybrid(result: dict) -> None:
    ...
    saved_target = evidence.get("target")
    event_label = {
        "Close[t+5] > Close[t]; 5 trading sessions": "Five trading sessions · Any positive terminal return",
        "Close[t+5] / Close[t] - 1 >= 0.03; 5 trading sessions": "Five trading sessions · ≥3% terminal gain",
        "Close[t+3] / Close[t] - 1 >= 0.02; 3 trading sessions": "Three trading sessions · ≥2% terminal gain (previous model)",
    }.get(saved_target, "Target unavailable")
    ...
    st.metric(
        ...
        help=(
            "XGBoost score for the saved model event: " + str(saved_target or "unavailable")
        ),
    )
    st.caption(
        event_label + " · "
        + ("Chronologically calibrated model score" if evidence.get("calibration") == "sigmoid"
